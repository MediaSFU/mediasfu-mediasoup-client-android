...
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO, "MSC-PC", __VA_ARGS__)
#else
...
namespace mediasoupclient
{
	  ...
	  PeerConnection::PrivateListener* privateListener, const PeerConnection::Options* options)
	{
		...
		LOGI("=== PeerConnection::PeerConnection ===");
		LOGI("  options ptr: %p", options);
		if (options) {
			LOGI("  options->factory ptr: %p", options->factory);
		}
		...
		{
			LOGI("  USING EXTERNAL FACTORY (with hardware decoders!)");
	...
	bool PeerConnection::RemoveTrack(webrtc::RtpSenderInterface* sender)
	{
		...
		// M137 removed RemoveTrack; use SetTrack(nullptr) instead
	...
	void PeerConnection::SetLocalDescriptionObserver::OnSetLocalDescriptionComplete(webrtc::RTCError error)
	{
		...
		{
			MSC_WARN(
			  "webrtc::SetLocalDescriptionObserver failure [%s:%s]",
	...
	void PeerConnection::SetRemoteDescriptionObserver::OnSetRemoteDescriptionComplete(webrtc::RTCError error)
	{
		...
		{
			MSC_WARN(
			  "webrtc::SetRemoteDescriptionObserver failure [%s:%s]",
			  std::string(webrtc::ToString(error.type())).c_str(),
			  error.message());
