...
namespace mediasoupclient
{
	  ...
	  const json* codec)
	{
		...
		rtc::scoped_refptr<webrtc::MediaStreamTrackInterface> trackRef(track);
		rtc::scoped_refptr<webrtc::RtpTransceiverInterface> transceiver =
		  this->pc->AddTransceiver(trackRef, transceiverInit);
		...
		// If VP8 or H264 and there is effective simulcast, add scalabilityMode to each encoding.
		// Use L1T3 (Layer 1, Temporal 3) for non-SVC codecs like VP8/H264.
		// S1T3 is for VP9 SVC only.
		auto mimeType = sendingRtpParameters["codecs"][0]["mimeType"].get<std::string>();
	  ...
	  const std::string& id, const std::string& kind, const json* rtpParameters)
	{
      ...
      localSdpObject["media"].begin(), localSdpObject["media"].end(), [&localId](const json& m) {
		...
		auto transceiverIt = std::find_if(
		  transceivers.begin(),
		  transceivers.end(),
		  [&localId](const rtc::scoped_refptr<webrtc::RtpTransceiverInterface>& t) { return t->mid() == localId; });
