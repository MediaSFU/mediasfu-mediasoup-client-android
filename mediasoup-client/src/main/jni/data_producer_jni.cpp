...
{
DataProducerListenerJni::DataProducerListenerJni(JNIEnv* env, const JavaRef<jobject>& j_listener)
{
	MSC_TRACE();
	(void)env;
...
webrtc::ScopedJavaLocalRef<jstring> JNI_DataProducer_GetId(JNIEnv* env, jlong j_producer)
{
	MSC_TRACE();
...
webrtc::ScopedJavaLocalRef<jstring> JNI_DataProducer_GetLocalId(JNIEnv* env, jlong j_producer)
{
	MSC_TRACE();
...
webrtc::ScopedJavaLocalRef<jstring> JNI_DataProducer_GetSctpStreamParameters(JNIEnv* env, jlong j_producer)
{
	MSC_TRACE();
...
webrtc::ScopedJavaLocalRef<jstring> JNI_DataProducer_GetLabel(JNIEnv* env, jlong j_producer)
{
	MSC_TRACE();
...
webrtc::ScopedJavaLocalRef<jstring> JNI_DataProducer_GetProtocol(JNIEnv* env, jlong j_producer)
{
	MSC_TRACE();
...
webrtc::ScopedJavaLocalRef<jstring> JNI_DataProducer_GetAppData(JNIEnv* env, jlong j_producer)
{
	MSC_TRACE();
  ...
  JNIEnv* env, jlong j_producer, const JavaParamRef<jbyteArray>& data, jboolean binary)
{
	...
	std::vector<int8_t> buffer = webrtc::JavaToNativeByteArray(env, data);
	reinterpret_cast<OwnedDataProducer*>(j_producer)
	  ->dataProducer()
	  ->Send(webrtc::DataBuffer(rtc::CopyOnWriteBuffer(buffer.data(), buffer.size()), binary));
}

webrtc::ScopedJavaLocalRef<jobject> NativeToJavaDataProducer(
