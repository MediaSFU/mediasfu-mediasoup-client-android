...
{
ProducerListenerJni::ProducerListenerJni(JNIEnv* env, const JavaRef<jobject>& j_listener)
{
	MSC_TRACE();
	(void)env;
...
static jlong JNI_Producer_GetTrack(JNIEnv* env, jlong j_producer)
{
	...
	// Keep WebRTC's sender-owned reference independent from the Java wrapper.
	if (result)
		result->AddRef();
